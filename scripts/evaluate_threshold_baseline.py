...
GRID_PATH = OUTPUT_DIR / "threshold_validation_grid.csv"
PER_IMAGE_PATH = (
    OUTPUT_DIR / "threshold_validation_best_per_image.csv"
)
CLASS_SUMMARY_PATH = (
    OUTPUT_DIR / "threshold_validation_best_by_class.csv"
)
BEST_CONFIG_PATH = (
    OUTPUT_DIR / "threshold_validation_best_config.json"
)

MASK_THRESHOLD = 128


def build_configs() -> list[ThresholdBaselineConfig]:
    configs = []

    for blur_kernel in [1, 3, 5, 7]:
        for invert in [False, True]:
            for morphology_kernel in [0, 3, 5]:
                configs.append(
                    ThresholdBaselineConfig(

...
## D010 - Freeze a Leakage-Aware 80/10/10 Dataset Split

Decision:

Use a deterministic train/validation/test split generated by
`scripts/build_splits.py` with random seed 42.

Final partition sizes:

- train: 3,673 samples
- validation: 459 samples
- test: 459 samples
- total: 4,591 samples
...
| 1 | 44 | 5 | 6 | 55 |
| 2 | 6 | 1 | 1 | 8 |
| 3 | 3,207 | 401 | 400 | 4,008 |
| 4 | 231 | 29 | 29 | 289 |
| 5 | 2 | 1 | 1 | 4 |
| 6 | 183 | 22 | 22 | 227 |

The confirmed near-duplicate group `near_duplicate_001` is treated as an
atomic unit and both samples were assigned to the training partition.

The generated `data/processed/splits.csv` is the frozen partition definition
for subsequent experiments.

Experimental usage:

- train: model fitting
- validation: hyperparameter selection, model selection, threshold tuning,
  and iterative error analysis
- test: final held-out evaluation

The test partition should not be repeatedly used to guide model-development
decisions.

Rare-class limitation:

Labels 2 and 5 contain only 8 and 4 total samples respectively. Although they
are represented in all three partitions, class-specific classification metrics
for these labels will have very high statistical uncertainty.
